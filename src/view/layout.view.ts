...
export function renderEffects(player: PlayerState): string {
    ...
    return effects.map(effect => {
        ...
        return `<span class="effect-icon${typeClass}" data-effect-id="${effect.id}" data-label="${effect.label}"${expiresAttr} title="${effect.label}"><span class="effect-emoji">${effect.icon}</span>${timerHtml}</span>`;
    }).join('');
}
