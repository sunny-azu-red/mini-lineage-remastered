...
- **The Board Reads At A Glance**: The first three overall wear 🥇🥈🥉, and they mean the same everywhere — filter to one lineage and its leader goes bare unless they are top three of *everyone*. A run that has not ended tints its row green, your own tints gold and wins where both apply, and a green `•` beside the name means somebody is online with that character **right now** — read from the process registry, so it costs no query. Times are stored as instants and rendered on the reader's own clock.
