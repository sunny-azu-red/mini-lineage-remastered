...
const localPlayer = (o: Partial<Parameters<typeof makePlayer>[0]> = {}) =>
    makePlayer({ health: 0, hpPercent: 0, lowHealth: true, dead: true, deathReason: 'You fought bravely... but not bravely enough.', highscoreEligible: true, ...o });
...
describe('DeathScreen', () => {
    ...
    it('shows the highscore submit button when highscoreEligible', () => {
        resetStore({ player: localPlayer({ highscoreEligible: true, coward: false, cheated: false }) });
        render(<DeathScreen />);
        expect(screen.getByRole('button', { name: /Write your Legacy!/ })).toBeInTheDocument();
    });

    it('hides the highscore submit button for a coward', () => {
        resetStore({ player: localPlayer({ highscoreEligible: false, coward: true, deathReason: '🤡 You took the cowardly way out.' }) });
        render(<DeathScreen />);
        expect(screen.queryByRole('button', { name: /Write your Legacy!/ })).not.toBeInTheDocument();
        // A danger alert for a coward OR a cheater; an ordinary paragraph for an honest death.
    ...
    it('hides the highscore submit button for a cheater even though they are not a coward', () => {
        resetStore({ player: localPlayer({ highscoreEligible: false, coward: false, cheated: true, deathReason: '👾 The gods saw your heresy.' }) });
        render(<DeathScreen />);
        expect(screen.queryByRole('button', { name: /Write your Legacy!/ })).not.toBeInTheDocument();
    ...
    it('submitting a highscore calls highscores:submit, re-hydrates, and resolves raceSlug to a raceFilter id', async () => {
        const freshHydrate: HydratePayload = { player: localPlayer({ started: false, dead: false, name: null }), catalog: localCatalog() };
        ...
        // localCatalog()), so HighscoresScreen lands pre-filtered to the submitter's own race —
    ...
    it('restarting calls game:restart and re-hydrates, routing to the actual reset-player shape (a non-null, started:false snapshot)', async () => {
        ...
        // stuck on 'death'.
        const freshHydrate: HydratePayload = { player: localPlayer({ started: false, dead: false, name: null, deathReason: null, highscoreEligible: false }), catalog: localCatalog() };
    ...
    // Defence in depth behind pinScreen: this screen offers "Play Again?", which wipes the
    // character, so it must be impossible to render for anyone who is not actually dead.
    it('renders nothing for a living character', () => {
        resetStore({ player: localPlayer({ dead: false, highscoreEligible: false }) });
        const { container } = render(<DeathScreen />);

        expect(container).toBeEmptyDOMElement();
    });

    it('renders nothing for a visitor with no character', () => {
        resetStore({ player: localPlayer({ started: false, dead: false, name: null }) });
        const { container } = render(<DeathScreen />);
