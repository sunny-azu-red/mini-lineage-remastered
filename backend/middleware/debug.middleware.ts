...
export const debugMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    res.on('finish', () => {
        ...
        let message = `[${req.method}:${formatSessionId(req.sessionID)}] \x1b[35m${req.url} = ${res.statusCode} (${Date.now() - start}ms)\x1b[0m`;

        if (req.method === 'POST' && req.body && Object.keys(req.body).length > 0)
            message += ` \x1b[33mPayload: ${JSON.stringify(req.body)}\x1b[0m`;

        if (req.session) {
            const { cookie, ...state } = { ...req.session } as Record<string, unknown>;
